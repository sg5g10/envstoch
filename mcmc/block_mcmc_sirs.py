    ...
    def run(self, prng_key):
        ...
        for i in range(self._iterations):
            ...
            if i==0:
                ...
                current_log_target, _x, _incidence = self._target(current, key)

            proposed_log_target, _x, _incidence = self._target(proposed, skey)
            ...
            if np.isfinite(proposed_log_target):
                if log_ratio > np.log(np.random.rand(1)):
                    accepted = 1
                    current = proposed
                    if i> self._iterations/2 and not np.any(_incidence)<0:
                        X.append(_x)
            ...
            if self._verbose and i % 2000 == 0:
                ...
                print('  Current params: ', np.std(chain[:i,:],axis=0))
                
                
        # Return generated chain
        if self._target._transform:
            return chain, X, incidence, tx_chain
        else:
            return chain, X, incidence
