...
import pickle
from models.logPDF_bma import LogPosteriorRegionWise
from models.forwardModel_bma import ForwardModelRegionWise
...
for reg_model in range(regions):
        ...
        regional_logPs.append(LogPosteriorRegionWise(forward_model, reg_model, data, sero_denom[:,:,reg_model]))
#######################################################################################
...
m_mul = np.array(np.split(np.loadtxt('./data/m_mul_starts.txt'),regions,axis=0)).reshape((regions,3)).T 

beta_rw = np.random.randn(10,7)
...
MCMCsampler = SubBlockRegionalAMGS(regional_logPs, x0, x0, \
                                   global_size=12, \
                                   regional_size=15, iterations=3000000)
t0 = timer.time()
trace = MCMCsampler.run()       
t1 = timer.time()
total = (t1 - t0) 
print('Time for SubBlock all regions AMGS BMA is: ',total)
