...
class LogPosteriorRegionWise(object):
    ...
    loglikelihood and lpg prior densities.
    ...
    with the Brownian motion approximation
    """        
    ...
    def __call__(self, parameters, joint_reg=False):
        ...
        beta_rw = parameters[17:27]  
        
        beta_rw = np.hstack((beta_rw, [beta_rw_sd]))
        ...
        # Now calculate the Brownian process likelihood (just priors on Coeffs)        
        log_bma_lik = np.sum(stats.norm.logpdf(beta_rw[:-1],0.0,1.0))
        
        log_likelihood =  log_death_lik + log_sero_lik + log_bma_lik
    ...
    def sample(self, parameters):
        ...
        beta_rw = parameters[17:27]  
        beta_rw = np.hstack((beta_rw, [beta_rw_sd]))
        expected_deaths, nnis, Rt = self._forward_model.simulate(dI, psir, beta_rw, I0, p_dead, m_mul, generate=True)  
